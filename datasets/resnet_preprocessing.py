...
def _decode_and_random_crop(image_bytes, image_size, seed, resize_method=None):
    ...
    decoded = image_bytes.dtype != tf.string
    original_shape = (
        tf.shape(image_bytes) if decoded else tf.image.extract_jpeg_shape(image_bytes)
    )
...
def preprocess_for_train(image_bytes, seed, image_size=IMAGE_SIZE, resize_method=None):
...
def preprocess_for_eval(image_bytes, image_size=IMAGE_SIZE, resize_method=None):
    """Preprocesses the given image for evaluation.
...
def preprocess_image(
    image_bytes, seed=None, is_training=False, image_size=IMAGE_SIZE, resize_method=None
):
    ...
    if is_training:
        return preprocess_for_train(
            image_bytes, seed=seed, image_size=image_size, resize_method=resize_method
        )
    else:
        return preprocess_for_eval(image_bytes, image_size, resize_method)
