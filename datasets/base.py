...
"""
    Abstract base class which defines interfaces for datasets.
...
_EnumeratedPreProcessFn = Callable[
    [int, Union[int, tf.Tensor, Sequence[tf.Tensor], types.Features]], types.Features
]


def get_validation_percent_split(
    dataset_builder, validation_percent, split, test_split=tfds.Split.TEST
):
...
class BaseDataset(robustness_metrics_base.TFDSDataset):
    ...
    def _create_process_example_fn(self) -> Optional[PreProcessFn]:
        ...
        Returns:
          ...
          Tensor values.
        """
        raise NotImplementedError("Must override dataset _create_process_example_fn!")
    ...
    def _load(
        self, *, preprocess_fn: Optional[PreProcessFn] = None, batch_size: int = -1
    ) -> tf.data.Dataset:
        ...
        dataset = dataset.map(
            preprocess_fn, num_parallel_calls=self._num_parallel_parser_calls
        )
